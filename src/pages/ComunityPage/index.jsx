...
const CommunityPage = () => {
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      for (const docSnapshot of querySnapshot.docs) {
        ...
        console.log(
          `Post válido encontrado: ${formattedPost.id}`,
          formattedPost
        );
        ...
        try {
          ...
          console.log(
            `${formattedPost.comments.length} comentários encontrados para o post ${formattedPost.id}`
          );
        } catch (commentError) {
          console.error(
            `Erro ao buscar comentários para o post ${formattedPost.id}:`,
            commentError
          );
  ...
  const fetchReports = async () => {
    try {
      ...
      const combinedData = [...reportsData, ...requestsData].sort(
    ...
    try {
      ...
      if (newPostImage) {
        const storageRef = ref(
          storage,
          `community_posts/${currentUser.uid}_${Date.now()}`
        );
  ...
  return (
    <div className="community-container">
      <header className="community-header">
        <h1>
          <FaLeaf /> Comunidade Ecottumaran
        </h1>
        <p>
          Conecte-se com outros defensores do meio ambiente e faça a diferença
          juntos!
        </p>
      </header>
            ...
            <div className="posts-list">
              {loading ? (
                ...
                <div className="no-content">
                  <p>
                    Ainda não há solicitações na comunidade. Seja o primeiro a
                    compartilhar!
                  </p>
                </div>
              ) : (
        ...
        {activeTab === "reports" && (
          <div className="reports-list">
            {loading ? (
              ...
              <div className="no-content">
                <p>
                  Não há denúncias recentes para exibir. Isso é bom para o meio
                  ambiente! 🌱
                </p>
              </div>
            ) : (
                ...
                <div className="report-card" key={report.id}>
                  ...
                  <div className="report-content">
                    ...
                    <div className="report-details">
                      {report.wasteType === "construction" && (
                        <>
                          <p>
                            ...
                            {report.quantity || "Não especificada"}
                          </p>
                          <p>
                            <strong>Frequência:</strong>{" "}
                            {report.frequency || "Não especificada"}
                          </p>
                          <p>
                            <strong>Possui PGRCC:</strong>{" "}
                            {report.hasPGRCC ? "Sim" : "Não"}
                          </p>
                        </>
                      )}

                      {report.serviceType === "denunciar" &&
                        report.responsible && (
                          <p>
                            <strong>Responsável pelo descarte:</strong>{" "}
                            {report.responsible}
                          </p>
                        )}
                      ...
                      {report.userContact && (
                        <p>
                          <strong>Contato:</strong> {report.userContact}
                        </p>
                      )}
                    </div>
      ...
      {!currentUser && (
        <div className="login-prompt">
          <p>
            Para participar da comunidade, compartilhar ideias ou fazer
            denúncias, faça login ou crie uma conta.
          </p>
          <button
